...
$t3 = gh issue create --repo $repo `
  ...
  --body "**Owner:** SA - Tommy Lammers (@voltron-1)`n**Priority:** CRITICAL | **Time:** 15 min`n`n## Description`nAdd the Team Members table to README.md using Appendix C format: Name | GitHub Username | Role - one row for each of the three members.`n`n## Acceptance Criteria`n- [x] Table added with Name, GitHub Username, Role columns`n- [x] Tommy Lammers | @voltron-1 | Security Analyst / Manager`n- [x] Sterling Garnett | @sterlinggarnett | System Architect / Engineer / Project Lead`n`n## Status`n**COMPLETED** - Committed to ``merge-to-main`` on 2026-04-29" `
  --milestone $milestone6 `
  --assignee "voltron-1" `
  --label "user-story"
Write-Host $t3
...
Write-Host "Creating Task 6 issue (PL - sterlinggarnett)..."
